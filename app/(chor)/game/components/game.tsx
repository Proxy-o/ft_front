...
const Game = ({
  ...
  useEffect(() => {
    ...
    if (leftUser?.username === onGoingGame.data?.game?.user1?.username) {
      while (
        (newAngleRef.current > (Math.PI * 1) / 4 &&
          newAngleRef.current < (Math.PI * 3) / 4) ||
        (newAngleRef.current > (Math.PI * 5) / 4 &&
          newAngleRef.current < (Math.PI * 7) / 4)
      ) {
        newAngleRef.current =
          Math.random() * Math.PI * (Math.random() > 0.5 ? 1 : -1);
      }
    ...
    function changeBallDirectionOnline() {
      if (canvas === null) return;
      if (
        newBallPositionRef.current.x < paddleWidth + ballRadius &&
        ...
        if (!ballInLeftPaddle) {
          ...
          if (newBallPositionRef.current.y < paddleLeftY + paddleHeight / 2) {
            newAngleRef.current =
              ((-Math.PI * 5) / 6) * (0.5 - ballPercentageOnPaddle);
          } else {
            newAngleRef.current =
              ((Math.PI * 5) / 6) * (ballPercentageOnPaddle - 0.5);
          }
    ...
    function changeBallDirectionOffline() {
      ...
      if (
        newBallPositionRef.current.x >
          canvas.width - paddleWidth - ballRadius &&
        ...
        if (!ballInRightPaddle) {
          ...
          if (
            newBallPositionRef.current.y <
            PaddleRightYRef.current + paddleHeight / 2
          ) {
            newAngleRef.current =
    ...
    function movePaddlesOffline() {
      ...
      } else if (wPressed && paddleLeftY > 0) {
        if (paddleLeftY - 20 < 0) {
          paddleLeftY = 0;
        } else {
          paddleLeftY -= 20;
    ...
    function changeBallDirectionOfflineTwo() {
      ...
      if (
        ...
        if (!ballInRightPaddle) {
          ...
          if (
            newBallPositionRef.current.y <
            PaddleRightYRefTwo.current + paddleHeight / 2
          ) {
            newAngleRef.current =
              ((-Math.PI * 2) / 6) * (0.5 - ballPercentageOnPaddle);
          } else {
