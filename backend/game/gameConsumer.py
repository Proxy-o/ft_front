...
class GameConsumer(WebsocketConsumer):
    ...
    def handle_debut(self, split):
        ...
        if game.type == "two" and (game.user1.status == 'offline' or game.user2.status == 'offline'):
            async_to_sync(self.channel_layer.group_send)(
                ...
                {'type': 'send_message', 'user': user1,
                    'message': f'/endGame {user1}'}
            )
            async_to_sync(self.channel_layer.group_send)(
                f'game_{game.user2.username}',
                {'type': 'send_message', 'user': user2,
                    'message': f'/endGame {user2}'}
            )
            game.delete()
    ...
    def handle_ready_to_start_four(self, split):
        ...
        async_to_sync(self.channel_layer.group_send)(
            f'game_{split[1]}',
            {'type': 'send_message',
                'user': split[1], 'message': f'/startFour {split[1]} {split[2]} {split[3]} {split[4]} {time}'}
        )
        async_to_sync(self.channel_layer.group_send)(
            f'game_{split[2]}',
            {'type': 'send_message',
                'user': split[2], 'message': f'/startFour {split[1]} {split[2]} {split[3]} {split[4]} {time}'}
        )
        async_to_sync(self.channel_layer.group_send)(
            f'game_{split[3]}',
            {'type': 'send_message',
                'user': split[3], 'message': f'/startFour {split[1]} {split[2]} {split[3]} {split[4]} {time}'}
        )
        async_to_sync(self.channel_layer.group_send)(
            f'game_{split[4]}',
            {'type': 'send_message',
                'user': split[4], 'message': f'/startFour {split[1]} {split[2]} {split[3]} {split[4]} {time}'}
        )
    ...
    def handle_end_game(self, split):
        user = self.user.username
        game = Game.objects.filter(Q(user1=self.user) | Q(
            user2=self.user) | Q(user3=self.user) | Q(user4=self.user)).last()
        if not game:
            return
        async_to_sync(self.channel_layer.group_send)(
            f'game_{game.user1.username}',
